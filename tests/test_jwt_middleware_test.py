...
async def test_missing_authorization_header(middleware):
    ...
    async def mock_send(message):
        if message["type"] == "http.response.start":
            ...
            assert message["more_body"] is False
            body = message["body"].decode()
            assert json.loads(body)["error"] == "Missing Authorization header"
...
async def test_invalid_token_format(middleware):
    ...
    async def mock_send(message):
        if message["type"] == "http.response.start":
            ...
            assert message["more_body"] is False
            body = message["body"].decode()
            assert json.loads(body)["error"] == "Invalid token format: Empty token"
...
async def test_expired_token(middleware):
    ...
    async def mock_send(message):
        if message["type"] == "http.response.start":
            ...
            assert message["more_body"] is False
            body = message["body"].decode()
            assert json.loads(body)["error"] == "Token has expired"
    ...
    for path in auth_paths:
        ...
        async def mock_send(message):
            if message["type"] == "http.response.start":
                ...
                assert message["more_body"] is False
                body = message["body"].decode()
                assert json.loads(body)["message"] == "success"
